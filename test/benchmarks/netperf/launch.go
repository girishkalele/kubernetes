...
func init() {
	...
	flag.StringVar(&tag, "tag", runUUID, "Storage Tag to use for this rug")
...
func setupClient() *client.Client {
	...
	if err != nil {
		fmt.Println("Failure loading kubeconfig credentials and server location")
		return nil
	}

	context, ok := kubeConfig.Contexts[kubeConfig.CurrentContext]
	if !ok {
		fmt.Println("Failed to find default kubectl context")
		return nil
	}
	authInfo, ok := kubeConfig.AuthInfos[fmt.Sprintf("%s-basic-auth", context.AuthInfo)]
	if !ok {
		fmt.Println("Failed to find the default auth info in the kubectl config file")
		return nil
	}
	...
	if err != nil {
		fmt.Println("Failed to connect to Kubernetes API server", err)
		return nil
	}
	return c
}

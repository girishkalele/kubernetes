...
func (kd *KubeDNS) Records(name string, exact bool) (retval []skymsg.Service, err error) {
	glog.V(4).Infof("Received DNS Request:%s, exact:%v", name, exact)
	trimmed := strings.TrimRight(name, ".")
	segments := strings.Split(trimmed, ".")
	isFederationQuery := false
	federationSegments := []string{}
	if !exact && kd.isFederationQuery(segments) {
		glog.V(4).Infof("federation service query: Received federation query. Going to try to find local service first")
	...
	if validRecord {
		...
		glog.V(4).Infof("federation service query: Returning CNAME for local service : %s", name)
...
func (kd *KubeDNS) getRecordsForPath(path []string, exact bool) ([]skymsg.Service, error) {
	...
	glog.V(4).Infof("records:%v, retval:%v, path:%v", records, retval, path)
	return retval, nil
}
...
func (kd *KubeDNS) ReverseRecord(name string) (*skymsg.Service, error) {
	glog.V(4).Infof("Received ReverseRecord Request:%s", name)
